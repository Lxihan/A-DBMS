...
void BackUp()
{
	...
	//以下共6个文件提供全体打包保存dblock.db,table.db,view.db,dic.db,log.txt
//*****************对dblock.db的备份********************************************
	int databaselock;
	if(dbislock())
		databaselock=1;
...
void DMLrecordinsert()
{

    clrscr();
    char large[MAX_TNAME];
    char tname[MAX_TNAME];
    char pro[MAX_PRONUM][MAX_PROLEN];
    char prova[MAX_PRONUM][MAX_PRONUM];
    int i,j,k;

	gotoxy(37,7);setcolor(0,14);

	printf("数据操纵语言");setcolor(0,15);
	...
	if(strcmp(substr(large,0,6),"insert")!=0)
	{
		printf("格式不对,没有发现insert关键字!!!任意键返回...\a");
		getch();
		return;
	}

	int x;
	for(x=0;x<(int)strlen(large);x++)
	{
		if(strcmp(substr(large,x,4),"into")==0)
			break;
		if(strcmp(substr(large,x,4),"into")!=0 && x>(int)strlen(large)-4)
		{
			printf("格式不对,没有发现into关键字!!!任意键返回...\a");
			getch();
			return;
		}
	}
	...
	for(x=0;x<(int)strlen(large);x++)
	{
		if(strcmp(substr(large,x,6),"values")==0)
			break;
		if(strcmp(substr(large,x,6),"values")!=0 && x>(int)strlen(large)-4)
		{
			printf("格式不对,没有发现values关键字!!!任意键返回...\a");
			getch();
			return;
		}
	}
		...
		for(q=0;q<t[p].pro_num;q++)
			{
			  j=0;
			    while(large[i]!=','&&large[i]!=')')
                   ...
                   prova[q][j]='\0';
                    strcpy(t[p].record[t[p].re_num][q],prova[q]);
                   if(large[i]==',')
                        i++;
                    //insert into r values (e,f);
                    cout<<t[p].record[t[p].re_num][q]<<endl;
			}
...
{   //
  ...
  //??
        for(int n=0;n<k;n++)
        {
        for (l=0;l<t[m].pro_num;l++)
        {
            if(strcmp(pro[n],t[m].proname[l])==0)
            {   j=0;
                  while(large[i]!=','&&large[i]!=')')
                {   //cout<<large[i];
                     prova[l][j++]=large[i++];
                     //cout<<large[i];
                    //insert into d (b,c) values (1,2);
                    //insert into d (a,c) values (1,2);
                }
                prova[l][j]='\0';
                //cout<<prova[l]<<endl;
                strcpy(t[m].record[t[m].re_num][l],prova[l]);
              // cout<<t[m].record[t[m].re_num][l]<<endl;succeed
                if(large[i]==',')
                    i++;
            }
        }
...
//insert into r (a,b) values (e,f);
//insert into r (a) values (e);
}
...
void DMLrecorddel()
{
    ...
    printf("DML记录删除操作");setcolor(0,15);
    DrawFrame(20,60,5,14);

    gotoxy(1,16);printf("\n输入DML语句: ");
    printf("例如delete from stu where sno<0005 and ssex!=male;\n\n\n>DML:");
    fflush(stdin);//清除缓冲区
    gets(large);
    //*******************************错误处理delete
    if(strcmp(substr(large,0,6),"delete")!=0)
	{
		printf("格式不对,没有发现delete关键字!!!任意键返回...\a");
		getch();
		return;
	}
	//*******************************错误处理from
	if(strcmp(substr(large,7,4),"from")!=0)
	{
	...
	//******************************错误处理where*********
	//int p;
	for(k=0;k<(int)strlen(large);k++)
	{
		if(strcmp(substr(large,k,5),"where")==0)
		{
		    break;
		}
		if(strcmp(substr(large,k,5),"where")!=0 && k>(int)strlen(large)-5)
		{
    ...
    int p;
	for(p=0;p<t_num;p++)
		if(strcmp(mn,t[p].tname)==0)
		{
			//i的值就是要删除的表的位置.
			goto eodrd;
		}
	//gotoxy(1,20);
	printf("未找到要删除记录的表,请确认输入.\a");
	getch();
	if(dbislock())
		dbunlock();
	return;
...
void recordupdate()
{
	...
	gotoxy(1,20);printf("未找到要修改的表,请确认输入.\a");
	getch();
	if(dbislock())
		dbunlock();
	return;
...
void DMLrecordupdate()
{
    ...
    char mn[MAX_TNAME];
	char tname[MAX_TNAME];
	char pro[MAX_PROLEN],shu[8];
	char pro1[MAX_PROLEN],shu1[8];
	char zj[MAX_PROLEN][MAX_PROLEN];
	char o1,o2;
	int i,j,k;
    gotoxy(33,7);setcolor(0,14);
    printf("DML记录修改操作");setcolor(0,15);
    DrawFrame(20,60,5,14);

    gotoxy(1,16);printf("\n输入DML语句: ");
    printf("例如update stu set sage=22 from stu,sc where sno<0004 and score>90 or cno==0001;\n");
    printf("\t     或者update stu set ssex=male where sno>=0002;\n\n\n>DML:");
    fflush(stdin);//清除缓冲区
    gets(large);
    //*******************************错误处理delete
    if(strcmp(substr(large,0,6),"update")!=0)
	{
		printf("格式不对,没有发现update关键字!!!任意键返回...\a");
		getch();
		return;
	}
	//*******************************错误处理set
	//以后添加其他出错处理，比如set where。
	int p;
	for(p=0;p<(int)strlen(large);p++)
	{
		if(strcmp(substr(large,p,3),"set")==0)
			break;
		if(strcmp(substr(large,p,3),"set")!=0 && p>(int)strlen(large)-4)
		{
			printf("格式不对,没有发现set关键字!!!任意键返回...\a");
			getch();
			return;
		}
	}
	...
	for(p=0;p<(int)strlen(large);p++)
	{
		...
		if(strcmp(substr(large,p,4),"from")!=0 && p>(int)strlen(large)-5)
		{
			//printf("格式不对,没有发现from关键字!!!任意键返回...\a");
			flag1 = 0;
			break;
    ...
    //添加错误处理
	for(k=0;k<t_num;k++)
		if(strcmp(mn,t[k].tname)==0)
		{
			//i的值就是要修改的表的位置.
			goto eodrd;
		}
    printf("未找到要修改记录的表,请确认输入.\a");
	getch();
	if(dbislock())
		dbunlock();
	return;
eodrd:;
    ...
    int q=0;
    for(q;q<t[k].pro_num;q++)
    {
        if(strcmp(t[k].proname[q],pro)==0)
        {
            break;
        }
    }
    for(int x=0;x<tn.re_num;x++)
    {
        for(int y=0;y<t[k].re_num;y++)
        {
            if(strcmp(t[k].record[y][t[k].key_no],tn.record[x][0])==0)
            {
                strcpy(t[k].record[y][q],shu);
            }
        }
    }
    printf("结果展示：\n");
...
//并操作，把T1表赋值给结果集
	...
	strcpy(tnew.tname,strcpy(t1->tname,t2->tname));
...
void relationsel0()
{
	...
	table tnew;		//结果集
	gotoxy(35,7);setcolor(0,14);
	printf("记录查询");setcolor(0,15);
	DrawFrame(20,60,5,14);
	gotoxy(27,9); printf("输入表名:");
	//gets(mn);
	cin>>mn;
	//添加错误处理
	for(i=0;i<t_num;i++)
...
void sqlrelationcalculate()
{
	...
	gotoxy(1,16);printf("\n输入sQL语句: ");
	printf("例如select sno,sage from stu,sc where sno<0003 and sage!=22 or sno==0004;\n\n\n>sQL:");
	fflush(stdin);//清除缓冲区
	gets(large);
	//*******************************错误处理select
	if(strcmp(substr(large,0,6),"select")!=0)
...
void tableDDL()
{
    ...
    clrscr();
    gotoxy(33,7);setcolor(0,14);
	printf("DDL语言操作");setcolor(0,15);
	DrawFrame(20,60,5,14);
	gotoxy(1,16);
	printf("\n请按照下列规则输入SQL语句: \n");
	cout << "添加新表: create table  表名" << endl;
    cout << "          (" << endl;
    cout << "          字段名称 字段类型(n/c)" << endl;
    cout << "          字段名称 字段类型(n/c)" << endl;
    cout << "          …        …" << endl;
    cout << "          primarykey 字段名称" << endl;
    cout << "          )" << endl;
    cout << "删除旧表: drop table 表名" << endl;
    cout << "修改表  :" << endl;
    cout << "          alter table 表名 add 字段名称 字段类型(n/c)" << endl;
    cout << "          alter table 表名 alter 字段名称 新字段名称 新字段类型(n/c)" << endl;
    cout << "          alter table 表名 drop 字段名称" << endl;
    printf("\n\n\n> DDL:\n");
	...
	while (cin >> db.cmd)
    {
        ...
        if (db.cmd == "create")
        {
           ...
           string tname;
           cin>>db.cmd>>tname;
           const char *tname1 = tname.c_str();
           strcpy(tnew.tname,tname1);
           cin>>tmp;
           db.transfer();
           while (tmp != ")")
            {
                ...
                {
                    cin>>tnew.protype[i];
                    //cout<<tnew.protype[i]<<endl;
                    const char *tmp1 = tmp.c_str();
                    strcpy(tnew.proname[i],tmp1);
                    //cout<<tnew.proname[i]<<endl;
                    a++;
                }
                i++;
            }
            tnew.pro_num=a;
            tnew.re_num=0;  //记录表设置为空
            for(int m=0;m<MAX_RECNUM;m++)
                for(int j=0;j<MAX_PRONUM;j++)
                    for(int k=0;k<MAX_PROLEN;k++)
                        tnew.record[m][j][k]='\0'; //初始化记录表
            t[t_num++]=tnew;
            printf("\n建表成功,回车返回上一级菜单.");
            getch();
            dbunlock();  //解锁@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            return;
            fprintf(alfp,"\n[%d年%d月%d日 %d:%d:%d]执行如下操作:\n",GetCurrentYear(),GetCurrentMonth(),GetCurrentDay(),GetCurrentHour(),GetCurrentMinute(),GetCurrentSecond() );
            fprintf(alfp,"用户建立表成功,表的内容是:表名:%s,表属性个数:%d,主键:%s.",tnew.tname,tnew.pro_num,tnew.key);fputc('\n',alfp);
            getch();
        }
        else if (db.cmd == "drop")
        {
            ...
            return;
            fprintf(alfp,"\n[%d年%d月%d日 %d:%d:%d]执行如下操作:\n",GetCurrentYear(),GetCurrentMonth(),GetCurrentDay(),GetCurrentHour(),GetCurrentMinute(),GetCurrentSecond() );
            fprintf(alfp,"用户成功删除一个表,表的内容是:表名:%s,表属性个数:%d,主键:%s.",tdel.tname,tdel.pro_num,tdel.key);
            fputc('\n',alfp);
            getch();
        }
        ...
        {
            int i,j,k;
            string tname,action,name,newname;
            char type,newtype;
			cin >> db.cmd >> tname >> action;
			...
			for(i=0;i<t_num;i++)
            {
                ...
                if(strcmp(mn,t[i].tname)==0)//确定表
                {
                    if(action == "add")
                    {
                        t[i].pro_num = t[i].pro_num + 1;//更改属性总个数
                        int a=t[i].pro_num;
                        cin>>name>>type;

                        t[i].protype[i]=type;
                        const char *name1 = name.c_str();
                        strcpy(t[i].proname[i],name1);
                        getchar();
                    ...
                    else if(action == "alter")
                    {
                        ...
                        {
                            //cout<<name1<<endl;
                            if(strcmp(t[i].proname[j],name1)==0) //确定属性
                            {
                                cin>>newname>>newtype;  //新属性名、新类型
                                t[i].protype[j]=newtype;

                                const char *newname1 = newname.c_str();
                                strcpy(t[i].proname[j],newname1);
                                getchar();
...
bha:;
	...
	gotoxy(27,13);printf("5.DDL语言操作  DATA DEFINITION");
	gotoxy(27,14);printf("0.返回上一级   RETURN UPPER");
...
bha:;
	...
	gotoxy(27,13);printf("5.DML记录插入  INSERT");
	gotoxy(27,14);printf("6.DML记录删除  DELETE");
	gotoxy(27,15);printf("7.DML记录修改  UPDATE");
	gotoxy(27,16);printf("0.返回上一级   RETURN UPPER");
	gotoxy(27,17);setcolor(0,11);printf("请选择:");setcolor(0,15);printf("[ ]");
	gotoxy(35,17);
	cin>>chr;
	switch(chr)
	{
		...
		case '3' :relationsel0();break;
		case '4' :recordupdate();break;
		case '5' :DMLrecordinsert();break;
		case '6' :DMLrecorddel();break;
		case '7' :DMLrecordupdate();break;
		case '0' :return;
		default:	gotoxy(1,16);
					printf("输入有误,请重新输入!\a");
					getchar();
...
bha:;
	...
	gotoxy(27,9);printf("1.建立视图     VIEW CREATE");
	gotoxy(27,10);printf("2.删除视图     VIEW DROP");
	gotoxy(27,11);printf("3.显示视图     VIEW SHOW");
	gotoxy(27,12);printf("0.返回上一级   RETURN UPPER");
	gotoxy(27,13);setcolor(0,11);printf("请选择:");setcolor(0,15);printf("[ ]");
	gotoxy(35,13);
	cin>>chr;
	switch(chr)
	{
		case '1' :viewcreate();break;
		case '2' :viewdrop();break;
...
bha:;
	...
	gotoxy(27,13);printf("5.数据库展示       SHOW");
...
bha:;
	...
	DrawFrame(20,60,5,21);
	gotoxy(27,10); printf("1.表的操作  建表，删表，查表");
	gotoxy(27,11);printf("2.记录操作  增，删，改");
	gotoxy(27,12);printf("3.视图操作");
	gotoxy(27,13);printf("4.关系代数操作");
	gotoxy(27,14);printf("5.关系演算查询功能");
	gotoxy(27,15);printf("6.备份全体数据");
	gotoxy(27,16);printf("7.恢复指定的某个备份");
	gotoxy(27,17);printf("0.退出");
	gotoxy(27,18);setcolor(0,11);printf("请选择:");setcolor(0,15);printf("[ ]");
	gotoxy(35,18);
	cin>>chr;
	switch(chr)
	{
        ...
        case '5' :sqlrelationcalculate();break;
		case '6' :BackUp();break;
		case '7' :ReCovery();break;
		case '0' :exit0();break;
		case 'a' :test();break;
		default:	gotoxy(1,24);
					printf("输入有误,请重新输入!\a");
